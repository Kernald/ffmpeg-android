                         ...
                         AVFrame *in_picture)
{
    ...
    case VSYNC_VSCFR:
        if (ost->frame_number == 0 && delta - duration >= 0.5) {
            LOGD("Not duplicating %d initial frames", (int)lrintf(delta - duration));
            delta = duration;
    ...
    if (nb_frames == 0) {
        ...
        if (nb_frames > dts_error_threshold * 30) {
            LOGE("%d frame duplication too large, skipping", nb_frames - 1);
            nb_frames_drop++;
            return;
        }
        nb_frames_dup += nb_frames - 1;
        LOGV("*** %d dup!", nb_frames - 1);
    }
  ...
  for (i = 0; i < nb_frames; i++) {
    ...
    if (s->oformat->flags & AVFMT_RAWPICTURE &&
        ...
        if (forced_keyframe) {
            ...
            LOGD("Forced keyframe at time %f", pts_time);
        }
        ...
        while (1) {
            ...
            if (ret < 0) {
                if (ret != AVERROR(EAGAIN) && ret != AVERROR_EOF) {
                    LOGW(
                           "Error in av_buffersink_get_frame_flags(): %s", av_err2str(ret));
                }
                break;
            }
            if (ost->finished) {
...
int guess_input_channel_layout(InputStream *ist)
{
    ...
    if (!dec->channel_layout) {
        ...
        LOGW("Guessed Channel Layout for  Input Stream "
               "#%d.%d : %s", ist->file_index, ist->st->index, layout_name);
    }
    return 1;
}
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    for (i = 0; i < ist->nb_filters; i++) {
        ...
        if (ret == AVERROR_EOF) {
            ...
            LOGE(
                   "Failed to inject frame into filter network: %s", av_err2str(ret));
            exit_program(1);
        }
    }
        ...
        if (ret < 0) {
            if (ist->hwaccel_id == hwaccel->id) {
                LOGE(
                       ...
                       "but cannot be initialized.", hwaccel->name,
                       ist->file_index, ist->st->index);
                exit_program(1);
            }
            continue;
...
static int transcode_init(void)
{
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        if (!oc->nb_streams && !(oc->oformat->flags & AVFMT_NOSTREAMS)) {
            ...
            LOGE("Output file #%d does not contain any stream", i);
            return AVERROR(EINVAL);
        }
    }
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (ost->forced_keyframes) {
                    if (!strncmp(ost->forced_keyframes, "expr:", 5)) {
                        ...
                        if (ret < 0) {
                            LOGE(
                                   "Invalid force_key_frames expression '%s'", ost->forced_keyframes+5);
                            return ret;
                        }
 ...
 dump_format:
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->filter && ost->filter->graph->graph_desc) {
            ...
            LOGI(" -> Stream #%d:%d (%s)", ost->file_index,
    ...
    if (key == 'c' || key == 'C'){
        ...
        if (k > 0 &&
            ...
            LOGE(
                   ...
                   "only %d given in string '%s'", n, buf);
        }
    }
    if (key == 'd' || key == 'D'){
        int debug=0;
    ...
    if (pkt.dts != AV_NOPTS_VALUE && ist->next_dts != AV_NOPTS_VALUE &&
        ...
        if (is->iformat->flags & AVFMT_TS_DISCONT) {
            ...
            if ( delta < -1LL*dts_error_threshold*AV_TIME_BASE ||
                ...
                LOGW("DTS %"PRId64", next:%"PRId64" st:%d invalid dropping", pkt.dts, ist->next_dts, pkt.stream_index);
                pkt.dts = AV_NOPTS_VALUE;
            }
            if (pkt.pts != AV_NOPTS_VALUE){
                ...
                if ( delta < -1LL*dts_error_threshold*AV_TIME_BASE ||
                    (delta > 1LL*dts_error_threshold*AV_TIME_BASE && ist->st->codec->codec_type != AVMEDIA_TYPE_SUBTITLE)) {
                    LOGW("PTS %"PRId64", next:%"PRId64" invalid dropping st:%d", pkt.pts, ist->next_dts, pkt.stream_index);
                    pkt.pts = AV_NOPTS_VALUE;
                }
            }
        }
...
int main(int argc, char **argv)
{
    ...
    if (nb_output_files <= 0 && nb_input_files == 0) {
        show_usage();
        LOGW("Use -h to get full help or, even better, run 'man %s'", program_name);
        exit_program(1);
		return main_return_code;
    }
