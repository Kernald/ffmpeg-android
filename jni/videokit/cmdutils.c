...
{
	// Required change for JNI. We mustn't exit.
                 ...
                 const OptionDef *options)
{
    ...
    if (!po->name) {
        LOGE("Unrecognized option '%s'", opt);
        return AVERROR(EINVAL);
    }
    if (po->flags & HAS_ARG && !arg) {
        LOGE("Missing argument for option '%s'", opt);
        return AVERROR(EINVAL);
    }
    ...
    for (i = 0; i < g->nb_opts; i++) {
        ...
        if (g->group_def->flags &&
            ...
            LOGE("Option %s (%s) cannot be applied to "
                   ...
                   "file it belongs to.", o->key, o->opt->help,
                   g->group_def->name, g->arg);
            return AVERROR(EINVAL);
        }
...
int opt_default(void *optctx, const char *opt, const char *arg)
{
    ...
    if ((o = opt_find(&fc, opt, NULL, 0,
        ...
        if (consumed)
            LOGV("Routing option %s to both codec and muxer layer", opt);
        consumed = 1;
    }
#if CONFIG_SWSCALE
    sc = sws_get_class();
    if (!consumed && opt_find(&sc, opt, NULL, 0,
        ...
        if (ret < 0) {
            LOGE("Error setting option %s.", opt);
            return ret;
        }
        consumed = 1;
    }
                      ...
                      const OptionGroupDef *groups, int nb_groups)
{
    ...
    while (optindex < argc) {
        ...
        if (opt[0] != '-' || !opt[1] || dashdash+1 == optindex) {
            ...
            LOGD(" matched as %s.", groups[0].name);
            continue;
        }
        opt++;
    ...
    if (!arg) {                                                                \
        ...
        if (po->name) {
            ...
            LOGD(" matched as option '%s' (%s) with "
                   "argument '%s'.", po->name, po->help, arg);
            continue;
        }
        ...
        if (argv[optindex]) {
            ...
            if (ret >= 0) {
                LOGD(" matched as AVOption '%s' with "
                       "argument '%s'.", opt, argv[optindex]);
                optindex++;
                continue;
            } else if (ret != AVERROR_OPTION_NOT_FOUND) {
                LOGE("Error parsing option '%s' "
                       "with argument '%s'.", opt, argv[optindex]);
                return ret;
            }
        }
...
int opt_loglevel(void *optctx, const char *opt, const char *arg)
{
    ...
    if (*tail) {
        LOGE("Invalid loglevel \"%s\". "
               "Possible levels are numbers or:", arg);
        for (i = 0; i < FF_ARRAY_ELEMS(log_levels); i++)
            LOGE("\"%s\"", log_levels[i].name);
        exit_program(1);
    }
...
static int init_report(const char *env)
{
    ...
    while (env && *env) {
        ...
        if (!strcmp(key, "file")) {
            ...
            LOGE("Unknown key '%s' in FFREPORT", key);
        }
        av_free(val);
        av_free(key);
    }
...
int opt_timelimit(void *optctx, const char *opt, const char *arg)
{
    ...
    if (setrlimit(RLIMIT_CPU, &rl))
        perror("setrlimit");
#else
    LOGW("-%s not implemented on this OS", opt);
#endif
    return 0;
}
...
int cmdutils_read_file(const char *filename, char **bufptr, size_t *size)
{
    ...
    if (!f) {
        LOGE("Cannot read file '%s': %s", filename,
               strerror(errno));
        return AVERROR(errno);
    }
    fseek(f, 0, SEEK_END);
    *size = ftell(f);
    fseek(f, 0, SEEK_SET);
    if (*size == (size_t)-1) {
        LOGE("IO error: %s", strerror(errno));
        fclose(f);
        return AVERROR(errno);
    }
