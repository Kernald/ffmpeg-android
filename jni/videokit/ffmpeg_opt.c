...
static int opt_map(void *optctx, const char *opt, const char *arg)
{
    ...
    if (sync = strchr(map, ',')) {
        *sync = 0;
        ...
        if (sync_file_idx >= nb_input_files || sync_file_idx < 0) {
            LOGE("Invalid sync file index: %d.", sync_file_idx);
            exit_program(1);
        }
        if (*sync)
            sync++;
    ...
    if (map[0] == '[') {
        ...
        if (!m->linklabel) {
            LOGE("Invalid output link label: %s.", map);
            exit_program(1);
        }
    } else {
        file_idx = strtol(map, &p, 0);
        if (file_idx >= nb_input_files || file_idx < 0) {
            LOGE("Invalid input file index: %d.", file_idx);
            exit_program(1);
        }
        if (negative)
    ...
    if (!m) {
        LOGE("Stream map '%s' matches no streams.", arg);
        exit_program(1);
    }
...
static void parse_meta_type(char *arg, char *type, int *index, const char **stream_spec)
{
    if (*arg) {
        *type = *arg;
        ...
        case 's':
            if (*(++arg) && *arg != ':') {
                LOGE("Invalid metadata specifier %s.", arg);
                exit_program(1);
            }
        ...
        case 'p':
            ...
            break;
        default:
            LOGE("Invalid metadata type %c.", *arg);
            exit_program(1);
        }
    } else
        *type = 'g';
...
#define SET_DICT(type, meta, context, index)\
    ...
    if (type_in == 's') {
        ...
        if (!meta_in) {
            LOGE("Stream specifier %s does not match  any streams.", istream_spec);
            exit_program(1);
        }
    }
...
static AVCodec *find_codec_or_die(const char *name, enum AVMediaType type, int encoder)
{
    ...
    if (!codec) {
        LOGE("Unknown %s '%s'", codec_string, name);
        exit_program(1);
    }
    if (codec->type != type) {
        LOGE("Invalid %s type '%s'", codec_string, name);
        exit_program(1);
    }
    return codec;
}
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        case AVMEDIA_TYPE_SUBTITLE: {
            ...
            if (canvas_size &&
                ...
                LOGE("Invalid canvas size: %s.", canvas_size);
                exit_program(1);
            }
            break;
        }
    ...
    if (!file_overwrite) {
        ...
        if (proto_name && !strcmp(proto_name, "file") && avio_check(filename, 0) == 0) {
            if (stdin_interaction && !no_file_overwrite) {
                ...
                LOGE("File '%s' already exists. Exiting.", filename);
                exit_program(1);
            }
        }
    }
...
static int open_input_file(OptionsContext *o, const char *filename)
{
    ...
    if (o->format) {
        if (!(file_iformat = av_find_input_format(o->format))) {
            LOGE("Unknown input format: '%s'", o->format);
            exit_program(1);
        }
    }
    ...
    if (ret < 0) {
        LOGE("%s: could not find codec parameters", filename);
    ...
    while ((e = av_dict_get(unused_opts, "", e, AV_DICT_IGNORE_SUFFIX))) {
        ...
        LOGW("Codec AVOption %s (%s) specified for "
               ...
               "which was not actually used for any stream.", e->key,
...
static OutputStream *new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index)
{
    ...
    while (bsf) {
        ...
        if (!(bsfc = av_bitstream_filter_init(bsf))) {
            LOGE("Unknown bitstream filter %s", bsf);
            exit_program(1);
        }
        if (bsfc_prev)
            bsfc_prev->next = bsfc;
...
static void parse_matrix_coeffs(uint16_t *dest, const char *str)
{
    ...
    for (i = 0;; i++) {
        ...
        p = strchr(p, ',');
        if (!p) {
            LOGE("Syntax error in matrix \"%s\" at coeff %d", str, i);
            exit_program(1);
        }
        p++;
    }
...
static OutputStream *new_video_stream(OptionsContext *o, AVFormatContext *oc, int source_index)
{
    ...
    if (frame_rate && av_parse_video_rate(&ost->frame_rate, frame_rate) < 0) {
        LOGE("Invalid framerate value: %s", frame_rate);
        exit_program(1);
    }
    ...
    if (frame_aspect_ratio) {
        ...
        if (av_parse_ratio(&q, frame_aspect_ratio, 255, 0, NULL) < 0 ||
            q.num <= 0 || q.den <= 0) {
            LOGE("Invalid aspect ratio: %s", frame_aspect_ratio);
            exit_program(1);
        }
    ...
    if (!ost->stream_copy) {
        ...
        if (frame_size && av_parse_video_size(&video_enc->width, &video_enc->height, frame_size) < 0) {
            LOGE("Invalid frame size: %s.", frame_size);
            exit_program(1);
        }
        ...
        if (frame_pix_fmt && (video_enc->pix_fmt = av_get_pix_fmt(frame_pix_fmt)) == AV_PIX_FMT_NONE) {
            LOGE("Unknown pixel format requested: %s.", frame_pix_fmt);
            exit_program(1);
        }
...
static OutputStream *new_audio_stream(OptionsContext *o, AVFormatContext *oc, int source_index)
{
    ...
    if (!ost->stream_copy) {
        ...
        if (sample_fmt &&
            ...
            LOGE("Invalid sample format '%s'", sample_fmt);
            exit_program(1);
        }
...
static int open_output_file(OptionsContext *o, const char *filename)
{
    ...
    if (!strcmp(file_oformat->name, "ffm") && !override_ffserver &&
        ...
        for(j = nb_output_streams - oc->nb_streams; j < nb_output_streams; j++) {
            ...
            if(!ost->sync_ist){
                LOGE("Missing %s stream which is required by this ffm", av_get_media_type_string(ost->st->codec->codec_type));
                exit_program(1);
            }
        }
...
loop_end:
    ...
    while ((e = av_dict_get(unused_opts, "", e, AV_DICT_IGNORE_SUFFIX))) {
        ...
        if (!(option->flags & AV_OPT_FLAG_ENCODING_PARAM)) {
            LOGE("Codec AVOption %s (%s) specified for "
                   "output file #%d (%s) is not an encoding option.", e->key,
    ...
    for (i = 0; i < o->nb_metadata_map; i++) {
        ...
        if (in_file_index >= nb_input_files) {
            LOGE("Invalid input file index %d while processing metadata maps", in_file_index);
            exit_program(1);
        }
        ...
        if (type == 's') {
            ...
            case 'c':
                if (index < 0 || index >= oc->nb_chapters) {
                    LOGE("Invalid chapter index %d in metadata specifier.", index);
                    exit_program(1);
                }
                m = &oc->chapters[index]->metadata;
                break;
            default:
                LOGE("Invalid metadata specifier %s.", o->metadata[i].specifier);
                exit_program(1);
            }
...
static int opt_target(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!strncmp(arg, "pal-", 4)) {
        ...
        if (norm != UNKNOWN)
            LOGI("Assuming %s for target.", norm == PAL ? "PAL" : "NTSC");
    }
...
static int opt_preset(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!(f = get_preset_file(filename, sizeof(filename), arg, *opt == 'f', codec_name))) {
        if(!strncmp(arg, "libx264-lossless", strlen("libx264-lossless"))){
            ...
            LOGE("File for preset '%s' not found", arg);
        exit_program(1);
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (!av_strtok(key,   "=",    &value) ||
            !av_strtok(value, "\r\n", &endptr)) {
            LOGE("%s: Invalid syntax: '%s'", filename, line);
            exit_program(1);
        }
        LOGD("ffpreset[%s]: set '%s' = '%s'", filename, key, value);
...
static int opt_channel_layout(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!layout) {
        LOGE("Unknown channel layout: %s", arg);
        return AVERROR(EINVAL);
    }
...
void show_help_default(const char *opt, const char *arg)
{
    ...
    if (opt && *opt) {
        if (!strcmp(opt, "long"))
            ...
            LOGE("Unknown help option '%s'.", opt);
    }
...
void show_usage(void)
{
    ...
    LOGI("usage: %s [options] [[infile options] -i infile]... {[outfile options] outfile}...", program_name);
}
                      ...
                      int (*open_file)(OptionsContext*, const char*))
{
    ...
    for (i = 0; i < l->nb_groups; i++) {
        ...
        LOGD("Opening an %s file: %s.", inout, g->arg);
